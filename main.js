...
phina.define("MainScene", {
  ...
  init: function(options) {
    ...
    (BLOCK_NUM).times(function(i) {
      ...
      var block = Block(angle).addChildTo(this.group).setPosition(90, 90);
...
phina.define('ComboLabel', {
  ...
  init: function(num) {
    ...
    // 数によって色とサイズを分岐
    if (num < 3) {
      this.fill = 'hsl(60, 60%, 60%)';
      this.fontSize = 16;
    }
    else if (num < 8) {
      this.fill = 'hsl(180, 72%, 72%)';
      this.fontSize = 28;
    }
    else if (num < 15) {
      this.fill = 'hsl(260, 85%, 85%)';
      this.fontSize = 42;
    }
    else {
      this.fill = 'hsl(320, 100%, 100%)';
      this.fontSize = 52;
    }
